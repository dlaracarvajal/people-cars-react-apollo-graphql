...
const resolvers = {
  Query: {
    ...
    personWithCars: (root, args) => {
      ...
      const personWithCars = carsArray.filter((car) => car.personId === person.id);
      return {
        person,
        cars: personWithCars,
      };
  ...
  Mutation: {
    ...
    removePerson: (root, args) => {
      ...
      remove(carsArray, (car) => car.personId === removedPerson.id);
    ...
    removeCar: (root, args) => {
      ...
      if (!removeCar) throw new Error(`Couldn't find car with id ${args.id}`);
